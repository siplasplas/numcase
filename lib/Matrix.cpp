...
Matrix::Matrix(int rows, int cols, const std::vector<float_type> &initArr)
                        :rows(rows),cols(cols) {
    if (initArr.size()!=rows*cols) throw std::exception();
    v = initArr;
}

float_type& Matrix::at(int row, int col) {
    return v[row * cols + col];
}

float_type Matrix::at(int row, int col) const{
    return v[row * cols + col];
}
...
void Matrix::ElimPartial(Vector &B) {
    ...
    {
        int max_row = diag;
        float_type max_val = fabs(at(diag, diag));
        float_type d;
        ...
        float_type invd = 1 / at(diag, diag);
